...
const CHARS_PER_TOKEN = 4; // rough token estimate: ~4 chars per token
...
function collectSessionSnapshot(): string {
  ...
  const lines = rows.map((r) => {
    const daysAgo = Math.round((Date.now() - r.last_active) / ONE_DAY_MS);
...
export function buildSnapshot(database: DatabaseInterface): string {
  ...
  const estimatedTokens = Math.ceil(snapshot.length / CHARS_PER_TOKEN);
  if (estimatedTokens > MAX_SNAPSHOT_TOKENS) {
    snapshot = snapshot.slice(0, MAX_SNAPSHOT_TOKENS * CHARS_PER_TOKEN) + '\n[...truncated]';
  }
