...
function buildUrgentActions(messageId: string): Array<{ label: string; action: string }> {
  const shortId = messageId.slice(0, SHORT_ID_LENGTH);
  return [
...
async function applyLabelAction(
  ...
  try {
    ...
    log.error(
      `Failed to label email ${email.messageId}: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}
...
async function applyArchiveAction(
  ...
  try {
    ...
    log.error(
      `Failed to archive email ${email.messageId}: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}

async function applyMarkReadAction(
  agentManager: AgentManagerLike,
  email: EmailPayload,
): Promise<string | null> {
  try {
    ...
    log.error(
      `Failed to mark email ${email.messageId} as read: ${err instanceof Error ? err.message : err}`,
    );
    return null;
  }
}

function emitExtractActionsEvent(email: EmailPayload): void {
  eventBus.emit({
    id: generateId(),
    timestamp: Date.now(),
    source: SUITE_EMAIL,
...
function applyUrgentFlagAction(email: EmailPayload): void {
  emitNotification(
...
async function executeTriageActions(
  ...
  if (actions.label) {
    const tag = await applyLabelAction(agentManager, email, actions.label);
    if (tag) actionsTaken.push(tag);
  }

  if (actions.archive) {
    const tag = await applyArchiveAction(agentManager, email);
    if (tag) actionsTaken.push(tag);
  }

  if (actions.markRead) {
    const tag = await applyMarkReadAction(agentManager, email);
    if (tag) actionsTaken.push(tag);
  }

  if (actions.extractActions) {
    emitExtractActionsEvent(email);
    actionsTaken.push('extractActions');
  }

  if (actions.flag === 'urgent') {
    applyUrgentFlagAction(email);
...
async function handleNewEmail(event: unknown): Promise<void> {
  ...
  log.info(
    `Email ${email.messageId} matched ${results.length} triage rule(s): ${results.map((r) => r.ruleName).join(', ')}`,
  );
