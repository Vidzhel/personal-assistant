...
function formatFromAddress(envelope: MessageEnvelopeObject): string {
  const first = envelope.from?.[0];
  if (!first) return 'Unknown';
  return `${first.name || ''} <${first.address || ''}>`.trim();
}

function buildNewEmailPayload(envelope: MessageEnvelopeObject): {
  from: string;
  subject: string;
  snippet: string;
  messageId: string;
  ...
  try {
    ...
    for await (const msg of client.fetch(range, { envelope: true, source: false })) {
      ...
      eventBus.emit({
        ...
        source: SOURCE_GMAIL,
        type: 'email:new',
        payload: buildNewEmailPayload(envelope),
      } as unknown);
    }
  } catch (err) {
