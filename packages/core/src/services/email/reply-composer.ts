...
const DRAFT_TTL_MS = 1800000; // 30 minutes
...
function emitNotification(title: string, body: string, options?: NotificationOptions): void {
  eventBus.emit({
    id: generateId(),
    timestamp: Date.now(),
    source: SUITE_EMAIL,
...
function buildComposePrompt(emailId: string, userIntent?: string): string {
  return userIntent
    ? [
...
function applyComposedDraft(
  ...
  if (!draftData) {
    ...
    emitNotification('Reply Failed', 'Could not parse the composed draft. Please try again.', {
      topicName,
    });
    pendingDrafts.delete(shortId);
    return;
  ...
  emitNotification(
    ...
    `To: ${draftData.to}\n\n${draftData.draftBody}`,
    { topicName, actions: buildDraftActions(shortId) },
  );
...
async function handleReplyStart(event: unknown): Promise<void> {
  ...
  try {
    ...
    applyComposedDraft(shortId, result, topicName);
  } catch (err) {
    log.error(`Reply composition error: ${err instanceof Error ? err.message : err}`);
    emitNotification('Reply Failed', 'An error occurred while composing the reply.', { topicName });
    pendingDrafts.delete(shortId);
  }
}

function buildSendPrompt(draft: PendingDraft): string {
  return [
...
async function handleReplySend(event: unknown): Promise<void> {
  ...
  try {
    ...
    if (result.success) {
      emitNotification('Reply Sent', `Your reply to "${draft.originalSubject}" has been sent.`, {
    ...
    emitNotification('Reply Failed', 'An error occurred while sending the reply.', {
      topicName: draft.topicName,
    });
  }
}

function buildEditPrompt(draft: PendingDraft, newInstructions: string): string {
  return [
...
function applyEditedDraft(
  ...
  if (!result.success || !result.result) {
    ...
    emitNotification('Edit Failed', 'Could not re-compose the draft. Please try again.', {
      topicName: draft.topicName,
    });
    return;
  }
  ...
  if (!draftData) {
    ...
    emitNotification('Edit Failed', 'Could not parse the updated draft.', {
      topicName: draft.topicName,
    });
    return;
  }
  ...
  emitNotification(
    ...
    `To: ${draftData.to}\n\n${draftData.draftBody}`,
    { topicName: draft.topicName, actions: buildDraftActions(getShortId(draft.compositionId)) },
  );
...
async function handleReplyEdit(event: unknown): Promise<void> {
  ...
  try {
    ...
    applyEditedDraft(compositionId, draft, result);
  } catch (err) {
    log.error(`Draft edit error: ${err instanceof Error ? err.message : err}`);
    emitNotification('Edit Failed', 'An error occurred while editing the draft.', {
      topicName: draft.topicName,
    });
  }
}
...
function handleReplyCancel(event: unknown): void {
  ...
  emitNotification('Reply Cancelled', 'The draft reply has been discarded.', { topicName });
}
...
function handlePermissionDenied(event: unknown): void {
  ...
  for (const shortId of pendingSendIds) {
    const draft = pendingDrafts.get(shortId);
    if (draft) {
      emitNotification('Reply Cancelled', 'Reply cancelled by approval denial.', {
        topicName: draft.topicName,
      });
      pendingDrafts.delete(shortId);
