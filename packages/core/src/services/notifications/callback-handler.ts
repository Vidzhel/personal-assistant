...
export function parseCallbackData(data: string): CallbackAction | null {
  ...
  if (parts.length < MIN_CALLBACK_PARTS) return null;
...
function emitApprovalOutcomeEvent(
  deps: CallbackDeps,
  outcome: { approval: PendingApprovalInfo; resolution: 'approved' | 'denied'; approvalId: string },
): void {
  const { approval, resolution, approvalId } = outcome;
...
function handleApprovalResolveError(err: unknown): CallbackResult {
  if (
...
function handleApprovalAction(action: CallbackAction, deps: CallbackDeps): CallbackResult {
  ...
  try {
    ...
    emitApprovalOutcomeEvent(deps, { approval, resolution, approvalId: action.target });
    ...
    return handleApprovalResolveError(err);
  }
}

function handleApprovalDetails(approvalId: string, deps: CallbackDeps): CallbackResult {
...
function handleEmailAction(action: CallbackAction, deps: CallbackDeps): CallbackResult {
...
function handleEmailReplyAction(action: CallbackAction, deps: CallbackDeps): CallbackResult {
...
function handleSnoozeAction(action: CallbackAction, deps: CallbackDeps): CallbackResult {
  if (!deps.db) {
  ...
  if (action.action === 'unsnooze') {
    return handleUnsnoozeAction(deps.db, action.target);
  }
...
function handleUnsnoozeAction(
  db: NonNullable<CallbackDeps['db']>,
  snoozeId: string,
): CallbackResult {
  // target is the snooze ID — look up category before deleting
  const snoozes = getActiveSnoozes(db);
  const snoozeRecord = snoozes.find((s) => s.id === snoozeId);

  const removed = removeSnooze(db, snoozeId);
  if (!removed) {
  ...
  if (snoozeRecord) {
    const snoozed = getSnoozedByCategory(db, snoozeRecord.category);
    if (snoozed.length > 0) {
      releaseSnoozed(
        db,
        snoozed.map((n) => n.id),
...
const DEFAULT_ADAPTIVE_THRESHOLD = 0.75;

// InsightRow is untyped here (insight-store.ts doesn't export its row type) —
// narrow to just the fields these handlers read.
interface InsightRowLike {
  service_sources: string;
  pattern_key: string;
}

function handleViewGraphInsightAction(
  target: { insightId: string; insight: InsightRowLike },
  db: NonNullable<CallbackDeps['db']>,
  deps: CallbackDeps,
): CallbackResult {
  const { insightId, insight } = target;
  const webUrl = process.env.RAVEN_WEB_URL ?? 'http://localhost:3000';
  updateInsightStatus(db, insightId, 'acted');
...
function handleInterestingInsightAction(
  insightId: string,
  db: NonNullable<CallbackDeps['db']>,
  deps: CallbackDeps,
): CallbackResult {
...
function bumpAdaptiveThresholdIfNeeded(
  domainPair: string,
  db: NonNullable<CallbackDeps['db']>,
  logger: CallbackDeps['logger'],
): void {
  const dismissalRow = db.get<{ cnt: number }>(
    'SELECT COUNT(*) AS cnt FROM cross_domain_dismissals WHERE domain_pair = ?',
    domainPair,
  );

  const dismissalCount = dismissalRow?.cnt ?? 0;
  if (dismissalCount < DISMISSAL_THRESHOLD) return;

  const existing = db.get<{ threshold: number }>(
    'SELECT threshold FROM cross_domain_thresholds WHERE domain_pair = ?',
    domainPair,
  );

  const currentThreshold = existing?.threshold ?? DEFAULT_ADAPTIVE_THRESHOLD;
  ...
  logger.info(`Adaptive threshold for ${domainPair} bumped to ${newThreshold}`);
}

function handleNotUsefulInsightAction(
  target: { insightId: string; insight: InsightRowLike },
  db: NonNullable<CallbackDeps['db']>,
  deps: CallbackDeps,
): CallbackResult {
  const { insightId, insight } = target;
  updateInsightStatus(db, insightId, 'dismissed');
  ...
  bumpAdaptiveThresholdIfNeeded(domainPair, db, deps.logger);
...
function handleKnowledgeInsightAction(action: CallbackAction, deps: CallbackDeps): CallbackResult {
  if (!deps.db) {
  ...
  const insight = getInsightById(db, insightId);
  if (!insight) {
    return { success: false, message: 'Insight not found' };
  }

  if (action.action === 'view-graph') {
    return handleViewGraphInsightAction({ insightId, insight }, db, deps);
  }

  if (action.action === 'interesting') {
    return handleInterestingInsightAction(insightId, db, deps);
  }

  if (action.action === 'not-useful') {
    return handleNotUsefulInsightAction({ insightId, insight }, db, deps);
  }
