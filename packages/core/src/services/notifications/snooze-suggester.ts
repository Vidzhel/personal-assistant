...
function hasRecentSuggestion(category: string): boolean {
  const cooldownCutoff = new Date(Date.now() - suggestionCooldownDays * ONE_DAY_MS).toISOString();
...
function proposeSnoozeForCategory(cat: CategoryStats): void {
  ...
  if (existingSnooze) return;

  // Skip if recently suggested
  if (hasRecentSuggestion(category)) return;
...
function checkForIgnoredCategories(): void {
  try {
    ...
    for (const cat of ignored) {
      proposeSnoozeForCategory(cat);
    }
  } catch (err) {
    log.error(
      `Snooze suggestion check failed: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
const service: RavenService = {
  async start(context: ServiceContext): Promise<void> {
    ...
    const checkInterval =
      (config.snoozeCheckIntervalMinutes as number) ?? DEFAULT_CHECK_INTERVAL_MINUTES;
    ...
    log.info(
      `Snooze suggester started (threshold: ${ignoreThreshold}, cooldown: ${suggestionCooldownDays}d, check every ${checkInterval}m)`,
    );
  },
