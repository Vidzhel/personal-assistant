...
const DEFAULT_ESCALATION_INTERVAL_MINUTES = 15;
...
export function recordDelivery(database: DatabaseInterface, notificationId: string): void {
  const id = generateId();
  database.run(
...
export function recordResponse(database: DatabaseInterface, notificationId: string | null): void {
  const id = generateId();
  database.run(
    `INSERT INTO engagement_metrics (id, event_type, notification_id, created_at)
     VALUES (?, ?, ?, ?)`,
...
function updateEngagementState(database: DatabaseInterface): void {
  ...
  if (previousState !== currentState) {
    ...
    log.info(
      `Engagement state changed: ${previousState} → ${currentState} (ratio: ${ratio.toFixed(2)})`,
    );
  }
}
...
const service: RavenService = {
  async start(context: ServiceContext): Promise<void> {
    ...
    const escalationInterval =
      config.escalationIntervalMinutes ?? DEFAULT_ESCALATION_INTERVAL_MINUTES;
    ...
    escalationJob = new Cron(`*/${String(escalationInterval)} * * * *`, () => {
      runEscalationCheck();
    });
