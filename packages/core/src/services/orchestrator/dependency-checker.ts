...
const UNKNOWN_SEVERITY_RANK = 4; // sorts after 'info' (the lowest known severity rank)
...
function parseAudit(jsonStr: string): Vulnerability[] {
  ...
  return results.sort((a, b) => {
    ...
    return (
      (order[a.severity] ?? UNKNOWN_SEVERITY_RANK) - (order[b.severity] ?? UNKNOWN_SEVERITY_RANK)
    );
  });
}
