...
function evaluateConcerns(
  ...
  if (healthStatus) {
    ...
    if (heapRatio > HEAP_WARN_PERCENT) {
      concerns.push(
        `Heap usage at ${(heapRatio * PERCENT_MULTIPLIER).toFixed(0)}% (${healthStatus.heapUsedMB.toFixed(0)}/${healthStatus.heapTotalMB.toFixed(0)} MB)`,
      );
    }

    if (healthStatus.failureRate !== undefined && healthStatus.failureRate > FAILURE_RATE_WARN) {
      concerns.push(
        `Task failure rate at ${(healthStatus.failureRate * PERCENT_MULTIPLIER).toFixed(1)}% (threshold: ${String(FAILURE_RATE_WARN * PERCENT_MULTIPLIER)}%)`,
      );
    }
