...
const FULL_COMPLIANCE_PERCENT_STR = '100';
...
function buildResourceSection(report: ResourceReport): string {
  ...
  if (report.healthStatus) {
    ...
    if (report.healthStatus.failureRate !== undefined) {
      lines.push(
        `- Task failure rate (1h): ${(report.healthStatus.failureRate * PERCENT_MULTIPLIER).toFixed(1)}%`,
      );
    }
  }
...
function buildConventionSection(report?: ConventionAuditReport): string {
  ...
  const pct =
    report.totalChecked > 0
      ? ((report.compliantCount / report.totalChecked) * PERCENT_MULTIPLIER).toFixed(0)
      : FULL_COMPLIANCE_PERCENT_STR;
  lines.push(
    `Compliance: ${report.compliantCount}/${String(report.totalChecked)} resources (${pct}%)`,
  );
