...
describe('GWS Drive Watcher Service', () => {
  ...
  beforeEach(async () => {
    ...
    vi.resetModules();
    const mod = await import('../../../services/google-workspace/drive-watcher.ts');
    service = mod.default;
  });
  ...
  it('logs warning and continues on CLI error', async () => {
    ...
    queueSpawnResponses({ code: 0, stdout: CHANGES_RESPONSE + '\n', stderr: '' });
  ...
  it('reads persisted pageToken on start', async () => {
    ...
    const startTokenCalls = mockSpawn.mock.calls.filter((call: any) =>
      call[1].includes('getStartPageToken'),
    );
    ...
    expect(mockSpawn).toHaveBeenCalledWith(
      ...
      expect.arrayContaining([
        ...
        'list',
        '--params',
        expect.stringContaining('persisted-token-99'),
      ]),
      expect.any(Object),
    );
  });
