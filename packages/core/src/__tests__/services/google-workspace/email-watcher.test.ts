...
describe('GWS Email Watcher Service', () => {
  ...
  beforeEach(async () => {
    ...
    vi.resetModules();
    const mod = await import('../../../services/google-workspace/email-watcher.ts');
    service = mod.default;
  });
  ...
  it('warns on invalid NDJSON lines', async () => {
    ...
    expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('Failed to parse NDJSON'));
