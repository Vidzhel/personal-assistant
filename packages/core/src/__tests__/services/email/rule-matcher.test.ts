...
import { matchRules, type EmailPayload } from '../../../services/email/rule-matcher.ts';
...
describe('rule-matcher', () => {
  describe('from matching', () => {
    it('matches sender containing pattern', () => {
      ...
      const rules = [
        makeRule({ name: 'noreply', match: { from: ['noreply@'] }, actions: { archive: true } }),
      ];
  ...
  describe('keyword (has) matching', () => {
    it('matches keyword in from field', () => {
      const email = makeEmail({ from: 'noreply@service.com' });
      const rules = [
        makeRule({ name: 'auto', match: { has: ['noreply'] }, actions: { archive: true } }),
      ];
    ...
    it('matches keyword in subject', () => {
      ...
      const rules = [
        makeRule({ name: 'unsub', match: { has: ['unsubscribe'] }, actions: { archive: true } }),
      ];
    ...
    it('matches keyword in snippet', () => {
      ...
      const rules = [
        makeRule({ name: 'auto', match: { has: ['automated'] }, actions: { markRead: true } }),
      ];
