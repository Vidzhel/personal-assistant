...
import {
  ...
  MAX_RETRY_ATTEMPTS,
  type ActionItem,
} from '../../../services/email/action-extractor.ts';
...
import service from '../../../services/email/action-extractor.ts';
...
const SINGLE_ACTION_ITEMS: ActionItem[] = [
  {
...
const MULTI_ACTION_ITEMS: ActionItem[] = [
  {
    title: 'Send the Q1 report',
    dueDate: '2026-03-20',
    priority: 'high',
...
describe('action-extractor service', () => {
  ...
  function setupEmailFetchAndExtraction(emailJson: object, actionItemsJson: unknown[]): void {
  ...
  describe('Task 3: AI-powered action item extraction', () => {
    it('calls gmail:search-emails with extraction prompt', async () => {
      ...
      const calls = mockAgentManager.executeAction.mock.calls;
    ...
    it('filters out items with empty titles', async () => {
      ...
      expect(mockAgentManager.executeAction).toHaveBeenCalledTimes(3);
      expect(mockAgentManager.executeAction).toHaveBeenCalledWith(
    ...
    it('handles malformed agent extraction response', async () => {
      mockAgentManager.executeAction
        ...
        .mockResolvedValueOnce({
          success: true,
          result: 'Sure! Here are the action items: not valid json',
        });
  ...
  describe('Task 4: create TickTick tasks', () => {
    ...
    it('creates multiple tasks for multiple action items (AC #2)', async () => {
      ...
      const ticktickCalls = mockAgentManager.executeAction.mock.calls.filter(
    ...
    it('queues for retry when ALL task creations fail (AC #4)', async () => {
      ...
      expect(entry.emailMeta).toEqual({
        from: 'boss@company.com',
        subject: 'Q1 Report',
        date: '2026-03-15',
      });
  ...
  describe('parseActionItems', () => {
    ...
    it('extracts JSON array from surrounding text', () => {
      const items = parseActionItems(
        `Here are the items: ${JSON.stringify(SINGLE_ACTION_ITEMS)} done.`,
      );
      expect(items).toHaveLength(1);
    });
    ...
    it('defaults missing priority to medium and missing context to empty', () => {
      const items = parseActionItems(JSON.stringify([{ title: 'Minimal item' }]));
      expect(items).toHaveLength(1);
  ...
  describe('edge cases', () => {
    it('handles concurrent extraction events', async () => {
      ...
      mockAgentManager.executeAction.mockImplementation((params: { actionName: string }) => {
