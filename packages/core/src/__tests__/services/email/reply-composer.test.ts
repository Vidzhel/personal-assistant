...
describe('reply-composer service', () => {
  ...
  beforeEach(async () => {
    ...
    const mod = await import('../../../services/email/reply-composer.ts');
    service = mod.default;
  });
  ...
  describe('pendingSendIds tracking (H2 fix)', () => {
    it('tracks compositionId in pendingSendIds on blocked send', async () => {
      ...
      const { pendingSendIds } = await import('../../../services/email/reply-composer.ts');
  ...
  describe('parseDraftResult', () => {
    it('parses valid JSON from agent response', async () => {
      const { parseDraftResult } = await import('../../../services/email/reply-composer.ts');
  ...
  describe('utility functions', () => {
    it('getShortId returns first 8 chars', async () => {
      const { getShortId } = await import('../../../services/email/reply-composer.ts');
      expect(getShortId('abcdefgh-1234-5678')).toBe('abcdefgh');
    });

    it('buildDraftActions creates correct callback data', async () => {
      const { buildDraftActions } = await import('../../../services/email/reply-composer.ts');
