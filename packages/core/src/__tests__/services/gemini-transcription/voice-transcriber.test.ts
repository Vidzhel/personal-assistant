...
describe('voice-transcriber service', () => {
  ...
  async function loadService() {
    const mod = await import('../../../services/gemini-transcription/voice-transcriber.ts');
    service = mod.default;
    return mod;
  }
  ...
  describe('stop and cleanup', () => {
    it('unsubscribes from event bus on stop', async () => {
      ...
      expect(mockEventBus.off).toHaveBeenCalledWith('transcription:request', expect.any(Function));
    });
  });
});
