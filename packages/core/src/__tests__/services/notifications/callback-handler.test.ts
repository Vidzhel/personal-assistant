...
import {
  parseCallbackData,
  handleCallback,
} from '../../../services/notifications/callback-handler.ts';
import type {
  CallbackAction,
  CallbackDeps,
  PendingApprovalInfo,
...
import { getActiveSnoozes } from '../../../notification-engine/snooze-store.ts';
...
describe('handleCallback', () => {
  ...
  describe('email actions', () => {
    ...
    it('updates keyboard after email action', () => {
      ...
      expect(result.updatedKeyboard).toEqual([
        [{ text: 'Archived \u2713', callback_data: 'noop' }],
      ]);
    });
  });
  ...
  describe('approval actions', () => {
    ...
    it('handles already resolved approval', () => {
      ...
      (deps.pendingApprovals.resolve as ReturnType<typeof vi.fn>).mockImplementation(() => {
        throw err;
      });
  ...
  describe('knowledge-insight actions', () => {
    ...
    it('not-useful dismisses and records domain-pair dismissal', () => {
      ...
      const mockDb: DatabaseInterface = {
        get: vi.fn().mockReturnValueOnce(mockInsight).mockReturnValueOnce({ cnt: 1 }),
        all: vi.fn().mockReturnValue([]),
        run: vi.fn(),
      };
      deps.db = mockDb;
