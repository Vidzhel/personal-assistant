...
const mockInsertInsight = vi.fn((..._args: any[]) => 'insight-id-1');
const mockFindRecentByHash = vi.fn();
const mockComputeSuppressionHash = vi.fn((..._args: any[]) => 'hash-abc');
const mockGetInsightsByStatus = vi.fn((..._args: any[]): any[] => []);
const mockUpdateInsightStatus = vi.fn();

vi.mock('../../../insight-engine/insight-store.ts', () => ({
...
import service from '../../../services/proactive-intelligence/insight-processor.ts';
...
describe('insight-processor', () => {
  ...
  beforeEach(async () => {
    ...
    const onCall = mockEventBus.on.mock.calls.find((c: any) => c[0] === 'agent:task:complete');
  ...
  describe('cross-domain insight handler', () => {
    ...
    it('creates insight with correct pattern_key and emits notification', () => {
      ...
      const notifEmit = mockEventBus.emit.mock.calls.find((c: any) => c[0].type === 'notification');
