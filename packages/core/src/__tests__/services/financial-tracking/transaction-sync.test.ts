...
// Mock ynab SDK (v4 shape: createTransaction singular, getPlanMonth, TransactionClearedStatus)
const mockCreateTransaction = vi.fn();
const mockGetTransactionsByAccount = vi.fn();
const mockGetPlanMonth = vi.fn();
...
describe('Financial Tracking — Transaction Sync', () => {
  ...
  describe('Monobank normalization', () => {
    it('normalizes kopecks → milliunits (×10) and generates correct import_id', async () => {
      const { normalizeMonobankTransactions } =
        await import('../../../services/financial-tracking/monobank-client.ts');
  ...
  describe('PrivatBank normalization', () => {
    it('normalizes decimal strings → kopecks (×100) and milliunits (×1000)', async () => {
      const { normalizePrivatBankTransactions } =
        await import('../../../services/financial-tracking/privatbank-client.ts');
  ...
  describe('YNAB push', () => {
    it('pushes transactions with correct import_id and milliunits', async () => {
      const { createYnabClient } =
        await import('../../../services/financial-tracking/ynab-client.ts');
      ...
      expect(mockCreateTransaction).toHaveBeenCalledWith('default', {
        transactions: [
  ...
  describe('YNAB category sync', () => {
    it('fetches categorized transactions and returns category data', async () => {
      ...
      const client = createYnabClient('test-token');
      const result = await client.fetchCategorizedTransactions('default', 'ynab-acc-1', {
  ...
  describe('Bank API error handling', () => {
    it('Monobank fetch failure throws with status code', async () => {
      ...
      await expect(
        fetchMonobankTransactions('token', { accountId: '0', from: 1711008000, to: 1711094400 }),
    ...
    it('PrivatBank fetch failure throws with status code', async () => {
      ...
      await expect(fetchPrivatBankTransactions('token', 'UA123', 30)).rejects.toThrow(
        'PrivatBank API 500',
      );
    });
  });
  ...
  describe('YNAB error isolation', () => {
    it('local storage succeeds even if YNAB push would fail', () => {
      ...
      const row = sqliteDb
        .prepare('SELECT * FROM financial_transactions WHERE bank_tx_id = ?')
        .get('mono-456');
      ...
      await client.pushTransactions('default', 'ynab-account-aaa', [
        {
          ...
          transactionDate: '2024-03-21',
          milliunits: -10000,
          importId: 'YNAB:-10000:2024-03-21:1',
          memo: null,
        },
      ]);
      ...
      await client.pushTransactions('default', 'ynab-account-bbb', [
        {
          ...
          transactionDate: '2024-03-21',
          milliunits: -20000,
          importId: 'YNAB:-20000:2024-03-21:1',
          memo: null,
        },
      ]);
  ...
  describe('YNAB month report', () => {
    it('fetches per-category budgeted vs actual breakdown', async () => {
      ...
      mockGetPlanMonth.mockResolvedValue({
        data: {
          month: {
            month: '2024-03-01',
            income: 5000000,
