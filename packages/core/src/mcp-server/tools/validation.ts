...
const SubmitValidationScoreSchema = {
  score: z
    .number()
    .int()
    .min(1)
...
// Single tool in this collection, so no schema-erasure is needed — the
// array element type stays the concrete, zod-inferred schema type.
export function buildValidationTools(
  deps: RavenMcpDeps,
  scope: ScopeContext,
): Array<SdkMcpToolDefinition<typeof SubmitValidationScoreSchema>> {
