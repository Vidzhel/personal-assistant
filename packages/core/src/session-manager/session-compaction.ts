...
export function createSessionCompaction(deps: SessionCompactionDeps): SessionCompaction {
  const { messageStore, eventBus, config, sessionBasePath } = deps;
  ...
  async function checkAndCompact(sessionId: string): Promise<boolean> {
    ...
    // Archive original transcript
    const sessionDir = join(sessionBasePath, sessionId);
    const transcriptPath = join(sessionDir, 'transcript.jsonl');
    if (existsSync(transcriptPath)) {
      const archiveName = `transcript-archived-${Date.now()}.jsonl`;
      renameSync(transcriptPath, join(sessionDir, archiveName));
    }

    // Write new transcript: compaction block + recent messages
    const newTranscript = [compactionMessage, ...recentMessages]
