...
- [H1] Fixed retrospective API returning incorrect bubble counts — `bubblesCreated`/`bubblesDrafted` now come from actual `processCandidateBubbles()` results, not re-counted from candidate list (which missed rejected/failed inserts)
- [M1] Session compaction now uses `MessageStore.archiveTranscript()` and `MessageStore.replaceTranscript()` instead of direct filesystem access — decoupled from JSONL storage format
