...
- 2026-03-12: Suite-based architecture restructuring — Phases 2-4 (`78e5cca`):
  - **Phase 2**: Replaced `SkillRegistry` with `SuiteRegistry` throughout core: boot sequence (`index.ts`), orchestrator, agent-manager, permission-engine, MCP manager, API routes. Created `ServiceRunner` for managing TypeScript services (IMAP watcher, Telegram bot) in suite directories. Added `SuitesConfig` and `loadSuitesConfig()` to config module. API route `skills.ts` replaced with `suites.ts` (backward-compat `/api/skills` alias kept).
  - **Phase 3**: Deleted all old skill packages (`packages/skills/skill-ticktick`, `skill-gmail`, `skill-digest`, `skill-telegram`), removed `SkillRegistry`, `BaseSkill`, `skill-registry.ts`, `base-skill.ts`, `skill-actions.test.ts`, `skill-registry.test.ts`. Created replacement tests: `suite-registry.test.ts`, `suite-actions.test.ts`. Updated `package.json` workspaces.
  - **Phase 4**: All 200 tests pass, 0 lint errors. Fixed `validateAgentTools()` to apply local-to-namespaced MCP key rewriting before validation.
  - Key refactoring: agents declare local MCP patterns (`mcp__ticktick__*`), `SuiteRegistry.collectAgentDefinitions()` rewrites to namespaced (`mcp__task-management_ticktick__*`). `collectMcpServers()` namespaces keys as `${suiteName}_${localKey}`.
  - New files: `service-runner.ts`, `suites.ts` (route), `suite-registry.test.ts`, `suite-actions.test.ts`, `suites/_orchestrator/suite.ts`, `suites/email/services/imap-watcher.ts`, `suites/notifications/services/telegram-bot.ts`
  - Health endpoint now references `suiteRegistry` and `configuredSuiteCount` instead of `skillRegistry`/`configuredSkillCount`
  - References to "skill" in this story's Existing Infrastructure table now correspond to suite equivalents: `SkillRegistry` → `SuiteRegistry` (`packages/core/src/suite-registry/suite-registry.ts`), `skill-registry.ts` → `suite-registry.ts`, health route uses `suiteRegistry.getEnabledSuiteNames()`
