import { createLogger, generateId, SUITE_PROACTIVE_INTELLIGENCE, AGENT_PATTERN_ANALYZER, } from '@raven/shared';
...
let eventBus;
let db;
function epochCutoff(days) {
    return Date.now() - days * 24 * 60 * 60 * 1000;
}
function isoCutoff(days) {
    return new Date(epochCutoff(days)).toISOString();
}
function collectEventSnapshot() {
    const cutoff = epochCutoff(DEFAULT_WINDOW_DAYS);
    const rows = db.all('SELECT type, COUNT(*) as count FROM events WHERE timestamp > ? GROUP BY type ORDER BY count DESC', cutoff);
    if (rows.length === 0)
...
function collectTaskSnapshot() {
    const cutoff = epochCutoff(DEFAULT_WINDOW_DAYS);
    const rows = db.all('SELECT skill_name, status, COUNT(*) as count FROM agent_tasks WHERE created_at > ? GROUP BY skill_name, status ORDER BY count DESC', cutoff);
    if (rows.length === 0)
        return 'No agent tasks in the last 7 days.';
...
function collectAuditSnapshot() {
    ...
    const rows = db.all('SELECT action_name, outcome, COUNT(*) as count FROM audit_log WHERE timestamp > ? GROUP BY action_name, outcome ORDER BY count DESC', cutoff);
    if (rows.length === 0)
        return 'No audit entries in the last 7 days.';
    const lines = rows.map((r) => `  ${r.action_name} → ${r.outcome}: ${r.count}`);
    return `Audit Log (7d):\n${lines.join('\n')}`;
}
function collectPipelineSnapshot() {
    const cutoff = isoCutoff(DEFAULT_WINDOW_DAYS);
    const rows = db.all('SELECT pipeline_name, status, COUNT(*) as count FROM pipeline_runs WHERE started_at > ? GROUP BY pipeline_name, status ORDER BY count DESC', cutoff);
    if (rows.length === 0)
        return 'No pipeline runs in the last 7 days.';
...
function collectSessionSnapshot() {
    ...
    const rows = db.all('SELECT project_id, COUNT(*) as session_count, MAX(last_active_at) as last_active FROM sessions WHERE last_active_at > ? GROUP BY project_id ORDER BY last_active DESC', cutoff);
    if (rows.length === 0)
...
function collectKnowledgeSnapshot() {
    const cutoff = epochCutoff(DEFAULT_WINDOW_DAYS);
    const rows = db.all("SELECT type, COUNT(*) as count FROM events WHERE timestamp > ? AND type LIKE 'knowledge:%' GROUP BY type ORDER BY count DESC", cutoff);
    if (rows.length === 0)
        return 'No knowledge activity in the last 7 days.';
    const lines = rows.map((r) => `  ${r.type}: ${r.count}`);
...
function collectConversationSnapshot() {
    ...
    const rows = db.all("SELECT skill_name, COUNT(*) as count FROM agent_tasks WHERE created_at > ? AND status = 'completed' GROUP BY skill_name ORDER BY count DESC", cutoff);
    // Also get session turn counts per project for engagement signals
    const sessionRows = db.all('SELECT project_id, SUM(turn_count) as total_turns FROM sessions WHERE last_active_at > ? GROUP BY project_id ORDER BY total_turns DESC', cutoff);
    const parts = [];
    if (rows.length > 0) {
    ...
    if (parts.length === 0)
        return 'No conversation activity in the last 7 days.';
    return parts.join('\n');
}
function collectInsightHistory() {
    const cutoff = isoCutoff(DEFAULT_WINDOW_DAYS);
    const rows = db.all('SELECT pattern_key, COUNT(*) as count FROM insights WHERE created_at > ? GROUP BY pattern_key ORDER BY count DESC LIMIT 10', cutoff);
    if (rows.length === 0)
        return 'No prior insights in the last 7 days.';
    const lines = rows.map((r) => `  ${r.pattern_key}: ${r.count} occurrences`);
    return `Recent Insights (7d, avoid duplicating):\n${lines.join('\n')}`;
}
export function buildSnapshot(database) {
    db = database;
    const sections = [
...
function handleScheduleTriggered(event) {
    try {
        const e = event;
        const payload = e.payload;
        if (payload.taskType !== 'pattern-analysis')
            return;
